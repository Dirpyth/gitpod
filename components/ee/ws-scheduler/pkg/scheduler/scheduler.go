...
type Scheduler struct {
	...
	localBindingCache *localSlotCache
...
func (s *Scheduler) startInformer(ctx context.Context) (schedulerQueue chan *corev1.Pod, newNodeQueue chan *corev1.Node, stopInformerQueue chan struct{}) {
	...
	s.localBindingCache = newLocalSlotCache()
...
func (s *Scheduler) schedulePod(ctx context.Context, pod *corev1.Pod) (err error) {
	...
	if isRegularWorkspace {
		reservedGhosts := s.localBindingCache.getReservedGhostsOnNode(nodeName)
	...
	s.localBindingCache.markAsScheduled(pod, nodeName, ghostToDelete)
